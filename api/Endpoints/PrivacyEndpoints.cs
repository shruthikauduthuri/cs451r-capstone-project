...
namespace api.Endpoints
{
    public static class PrivacyEndpoints
    {
        ...
        {
            app.MapGet("api/privacy", async (Client supabase) =>
            {
                var privacyPolicy = await supabase.From<PrivacyRule>().Get();
                return Results.Ok(privacyPolicy);
            });
            
            app.MapGet("api/privacy/{id}", async (Client supabase, string id) =>
            {
             ...
             app.MapPost("api/privacy", async (Client supabase, CreatePrivacyRuleRequest request) =>
            {
             ...
             app.MapPut("api/privacy/{id}", async (Client supabase, string id, PrivacyRule request) =>
            {
                await supabase.From<PrivacyRule>().Where(p => p.Id == id).Update(request);
                return Results.Ok();
            });

             app.MapDelete("api/privacy/{id}", async (Client supabase, string id) =>
            {
